...
function Education() {
  return (
    <section id="education" className="bg-slate-950 px-6 py-24 sm:px-8 lg:px-12">
      <div className="mx-auto max-w-5xl">
        {/* HEADING */}
        <div className="mb-16 text-center">
          <p className="mb-3 text-sm font-semibold uppercase tracking-[0.3em] text-cyan-400">Education</p>
          <h2 className="text-3xl font-bold text-white sm:text-4xl md:text-5xl">Academic Journey</h2>
        ...
        {education.length > 0 && (
          <div className="relative">
            ...
            {education.map((item, index) => (
              <div key={item.id} className={`relative mb-14 flex w-full items-center ${index % 2 === 0 ? "sm:justify-start" : "sm:justify-end"}`}>
                {/* DOT */}
                  ...
                  <div className="group relative overflow-hidden rounded-2xl border border-white/10 bg-slate-900/70 p-6 shadow-xl shadow-black/20 backdrop-blur-sm transition-all duration-500 hover:-translate-y-2 hover:border-cyan-400/50 hover:shadow-cyan-400/10">
                    ...
                    <div className="relative flex flex-wrap items-center gap-3">
                      <span className="rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-xs font-semibold text-cyan-400">
                        {item.start_year || "N/A"} - {item.end_year || "Present"}
                      </span>
                      {item.grade && (
                        <span className="rounded-full border border-white/10 bg-slate-800 px-3 py-1 text-xs text-slate-400">{item.grade}</span>
                      )}
                    </div>
                    <h3 className="relative mt-5 text-xl font-bold text-white transition-colors duration-300 group-hover:text-cyan-300 sm:text-2xl">{item.degree}</h3>
                    <p className="relative mt-2 font-medium text-slate-300">{item.institution}</p>
                    {item.location && <p className="relative mt-2 text-sm text-cyan-400/80">📍 {item.location}</p>}
                    {item.description && <p className="relative mt-4 text-sm leading-7 text-slate-400 sm:text-base">{item.description}</p>}
